...
FROM
    employees
WHERE
    hire_date BETWEEN '01-JAN-03' AND '31-DEC-03';
...
SELECT DISTINCT
    first_name                         AS "이름",
    nvl(to_char(commission_pct), '없어') AS "커미션"
FROM
    employees;
...
SELECT
    translate('2021-06-07', '-', '/')
FROM
    dual;

SELECT
    replace('2021-06-07', '-0', 'pp')
FROM
    dual;

SELECT
    translate('2021-06-07', '-0', 'ab')
FROM
    dual;
...
FROM
    employees
WHERE
    translate(email, '0123456789', '__________') <> email;
...
SELECT
    round(to_date('16NOV21'), 'mon')
FROM
    dual;

SELECT
    round(TO_DATE('12 APR 2021', 'dd mm yyyy'), 'mon')
FROM
    dual;
...
FROM
    dual; -- 월을 더함
SELECT
    months_between(sysdate, to_date('12APR19'))
FROM
    dual; -- 둘사이 개월수 (recent, old)
...
SELECT
    first_name                                        AS "이름",
    trunc(months_between(sysdate, hire_date) / 12, 1) AS "근무년수"
FROM
    employees; -- 지금까지 근무년수
...
FROM
    employees
WHERE
    trunc(months_between(sysdate, hire_date) / 12) >= 15;

SELECT
    to_char(sysdate, 'yyyy/mm/dd') AS "날짜"
FROM
    dual; -- 출력은 문자형태로 가능하지만 db에는 RR/mm/dd로 저장되어있음
SELECT
    first_name AS "이름",
    hire_date  AS "입사일"
FROM
...
SELECT
    AVG(commission_pct)
FROM
    employees; -- commission percent의 null은 제외하고 계산됨
SELECT
    SUM(commission_pct)
FROM
    employees;
...
SELECT
    ...
    COUNT(*)                      AS "직원수",
    COUNT(manager_id)             AS "매니저있는 직원",
    COUNT(DISTINCT department_id) AS "직원부서 수"
FROM
    employees;
...
SELECT
    ...
    e.last_name                      AS "성",
    e.hire_date                      AS "입사일",
    nvl(d.department_name, '그런부서없어') AS "부서명"
FROM
    employees   e,
    departments d
WHERE
        e.department_id = d.department_id (+)
    AND substr(e.hire_date, 8, 2) LIKE '07'
ORDER BY
    e.last_name ASC;
...
FROM
    ...
    LEFT OUTER JOIN departments d ON ( e.department_id = d.department_id )
WHERE
    substr(e.hire_date, 8, 2) LIKE '07'
ORDER BY
    e.last_name ASC;
...
FROM
    employees e
    FULL JOIN employees d ON ( e.manager_id = d.employee_id )
ORDER BY
    e.employee_id ASC;
...
FROM
         employees e
    INNER JOIN salgrade s ON ( e.salary BETWEEN lsal AND hsal )
ORDER BY
    e.salary DESC,
    e.first_name ASC;
...
SELECT
    e.first_name                                                            AS "이름",
    j.job_title                                                             AS "직책",
    decode(lower(j.job_title), '%manager%', e.salary * 1.1, e.salary * 1.5) AS "인상된급여"
FROM
    employees e,
    jobs      j
WHERE
...
SELECT
    ...
    e.job_id                               AS "잡아이디",
    e.salary                               AS "급여",
    decode(substr(e.job_id, 1, 2), 'AD', e.salary * 1.1, 'FI', e.salary * 1.15,
           'PU', e.salary * 1.2, e.salary) AS "인상된급여"
FROM
    employees e,
    jobs      j
WHERE
...
SELECT
    ...
    CASE
        WHEN j.job_title LIKE '%Manager' THEN
            e.salary * 1.1
        WHEN j.job_title LIKE '%Accountant' THEN
            e.salary * 1.2
        ELSE
            e.salary * 1.3
    END          AS "인상된급여"
...
CREATE TABLE memidtable (
    memid   NUMBER(6)
        CONSTRAINT memidtable_memid_pk PRIMARY KEY,
    memname VARCHAR2(20)
);
-- column level 방식
...
CREATE TABLE memtable (
    ...
    CONSTRAINT memtable_membi_ck CHECK ( membi > 1000 ),
    CONSTRAINT memtable_memid_fk FOREIGN KEY ( memid )
        REFERENCES memidtable ( memid )
);
...
CREATE OR REPLACE VIEW dpt_100_view AS
    ( SELECT
        first_name,
        phone_number
    FROM
...
SET
    manager_id = 150
WHERE
    employee_id = 131; -- check option때문에 안됨
-- view 수정 자체를 금지
CREATE OR REPLACE VIEW dpt_100_view AS
    ( SELECT
        *
